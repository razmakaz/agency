	...
	const payload = [
		{
			...
			custom_domain: 'http://localhost:5143',
			subdomain: 'http://subdomain.localhost:5143',

			website_url: 'http://localhost:5143'
		}
	];
	...
	for await (const whiteLabel of payload) {
		try {
			const inserted = await db
				.insert(white_label_configs)
				.values(whiteLabel)
				.onConflictDoUpdate({
					target: white_label_configs.id,
					set: {
						...whiteLabel
					}
				})
			...
			console.error(
				`Error seeding white label config ${whiteLabel.id}:`,
